// ---- Console page — decision feed, HITL queue, live counters, scenarios ----
// Owns all console state (metrics, feed, queue). In LIVE mode the real proxy
// drives everything (refreshLiveData, exported so the connection poll can call
// it); in SIMULATED mode the scenario narrations below stand in.
import { inr, stamp, p95, pick } from "./format.js";
...
function renderQueue() {
  ...
  if (qCount) qCount.textContent = queue.length + " pending";
  if (!q) return;
...
// ---- decision helpers (simulated mode) ----
function emit(ev) {
...
async function scnStructuring() {
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    if (sum >= WINDOW_THRESHOLD) {
      ...
      await wait(400);
      emit({ agent: AGENT, tool: "issue_refund", verdict: "escalate", cls: "escalate", policy: "velocity_aggregation",
...
function resetAll() {
  ...
  events = []; queue = []; seq = 0;
  const feed = document.getElementById("feed");
  if (feed) feed.innerHTML = '<div class="empty">Press a scenario above — every tool call the agent attempts is intercepted and evaluated here before it reaches Razorpay.</div>';
  const count = document.getElementById("feedCount");
...
// Real decisions/reasons come back from the actual policy engine, so — unlike
// the simulated narration above — these don't hardcode a verdict;
// refreshLiveData() after each call pulls the true result into the feed.
...
// ---- LIVE data refresh (called by the connection poll + after each op) ----
function normalizeMetrics(raw) {
  // GET /metrics returns money in RUPEES (rupees_attempted/rupees_moved — the
  // one display-layer exception to paise-everywhere); multiply back by 100.
  return {
...
export async function refreshLiveData() {
  ...
  if (decisionsRes.status === "fulfilled") {
    ...
    // Assume /decisions returns newest-first — reverse so pushEvent's
    // prepend-to-top restores newest-first display for un-rendered items.
...
// ---- wire the scenario toolbar + first render --------------------------------
